...
def main(
    ...
    if mode == "record":
        ...
        paths = [
            None,
        ] * num_repeat  # empty paths for recordings
    else:
        ...
        for i_path, path in enumerate(paths):
            ...
            if path:
                ...
                if output_type == ".h5":
                    ...
                    env.reset(
                        reset_qpos=path["env_infos"]["state"]["qpos"][0],
                        reset_qvel=path["env_infos"]["state"]["qvel"][0],
                    )
                else:
            ...
            for i_step in range(path_horizon):
                ...
                # Apply actions in open loop
                    ...
                    if output_type == ".h5":
                        a = np.concatenate(
                            [data["ctrl_arm"][i_step], data["ctrl_ee"][i_step]]
                        )
                    else:
                        a = (
                            path["actions"][i_step]
                            if output_type == ".pickle"
                            else path["data"]["ctrl_arm"]
                        )
                    onext, r, d, info = env.step(a)  # t ==> t+1
                ...
                # Recover actions from states
                    ...
                    if noise_scale:
                        a = a + env.env.np_random.uniform(
                            high=noise_scale, low=-noise_scale, size=len(a)
                        ).astype(a.dtype)
                ...
                # Render offscreen
                if render == "offscreen":
                    curr_frame = env.mj_renderer.render_camera_offscreen(
                        cameras=[camera_name],
                        width=frame_size[0],
                        height=frame_size[1],
                        device_id=0,
            ...
            # Finish rollout
            print(
                "-- Finished playback path %d :: Total reward = %3.3f, Total time = %2.3f"
                % (i_path, ep_rwd, ep_t0 - time.time())
            )
    ...
    if save_paths:
        file_name = os.path.join(
            output_dir, output_name + "{}_paths.pickle".format(time_stamp)
        )
